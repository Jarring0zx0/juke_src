...
template std::shared_ptr<JukeUI> ManagerComponent::CreateModule<JukeUI>(Module* mod, std::shared_ptr<JukeUI>& moduleToBind);
template std::shared_ptr<JukeUI> ManagerComponent::GetModule<JukeUI>(const std::string& moduleName);
...
void ManagerComponent::Initialize() {
    std::shared_ptr<JukeUI> jukeUI;
    CreateModule<JukeUI>(new JukeUI("JukeUI", "The main UI for Juke", States::STATES_All), jukeUI);
}
